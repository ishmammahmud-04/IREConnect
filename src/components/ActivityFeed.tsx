      ...
      {entries.map((entry) => {
        ...
        return <article key={`${entry.type}-${entry.id}`} className="rounded-xl border border-slate-200 p-3.5">
          <div className="flex items-start gap-2">
            ...
            <button onClick={() => submitReport(entry.title, 'Inappropriate Content', `Reported from activity feed (${entry.type}).`, entry.type, entry.id)} className="text-slate-400 hover:text-rose-600" aria-label="Report content"><span className="material-symbols-outlined text-[18px]">flag</span></button>
          </div>
