...
export const AchievementDetailModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex justify-center items-start overflow-y-auto p-2 sm:p-4 md:p-6 animate-in fade-in duration-200" role="dialog" aria-modal="true" aria-label={`Achievement details: ${selectedAchievement.title}`}>
        ...
        <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-md px-5 py-2.5 border-b border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              aria-label={isSaved ? 'Remove achievement bookmark' : 'Save achievement bookmark'}
            ...
            {canManage && (
              <>
                <button type="button" onClick={handleEdit} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors" title="Edit achievement" aria-label="Edit achievement"><span className="material-symbols-outlined text-[18px]">edit</span></button>
                <button type="button" onClick={() => void handleDelete()} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-colors" title="Delete achievement" aria-label="Delete achievement"><span className="material-symbols-outlined text-[18px]">delete</span></button>
              </>
            )}
          </div>
        </div>
