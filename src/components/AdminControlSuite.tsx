...
export const AdminControlSuite: React.FC = () => {
  ...
  const accountRoles = Array.from(new Set(accountUsers.map((user) => user.role)));
  const accountStatuses = Array.from(new Set(accountUsers.map((user) => user.verificationStatus)));
  const normalizedAccountSearch = accountSearch.trim().toLowerCase();
  const filteredAccountUsers = accountUsers.filter((user) => {
    const matchesSearch =
      !normalizedAccountSearch ||
      [user.name, user.email, user.department].some((value) => value.toLowerCase().includes(normalizedAccountSearch));
    const matchesRole = accountRoleFilter === 'all' || user.role === accountRoleFilter;
    const matchesStatus = accountStatusFilter === 'all' || user.verificationStatus === accountStatusFilter;
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-2xs space-y-3">
            <div className="flex flex-col md:flex-row gap-2">
              <label className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-200 py-2 pl-9 pr-3 text-xs text-slate-900 focus:outline-none focus:border-blue-600"
                />
              </label>
              <label>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 focus:outline-none focus:border-blue-600"
                >
                  <option value="all">All roles</option>
                  {accountRoles.map((role) => <option key={role} value={role}>{role.replace('_', ' ')}</option>)}
                </select>
              </label>
              <label>
            ...
            <p className="text-[11px] text-slate-500">
              Showing {filteredAccountUsers.length} of {accountUsers.length} account{accountUsers.length === 1 ? '' : 's'}
            </p>
          </div>

          {filteredAccountUsers.length > 0 ? filteredAccountUsers.map((user) => (
            <div key={user.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4">
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] text-slate-400">{user.department || 'Department unavailable'} · {user.verificationStatus}</p>
              </div>
              <button
                type="button"
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-2xs">
              <span className="material-symbols-outlined text-[32px] text-slate-400">manage_search</span>
              <h3 className="font-heading text-sm font-bold text-slate-900">
                {accountUsers.length ? 'No matching accounts' : 'No accounts available'}
              </h3>
              <p className="text-xs text-slate-500">
                {accountUsers.length ? 'Try adjusting the search or filters.' : 'There are no user accounts to manage.'}
              </p>
            </div>
          )}
        </div>
